...
export default function TentangSekolah() {
  ...
  return (
    <div className="flex h-full flex-col gap-0 rounded-3x bg-white p-48">
      {loadingTentangSekolah ? (
        ...
        <>
          ...
          {/* --- Konten --- */}
          <div className="scrollbar flex h-full flex-1 overflow-y-auto pt-12">
            {menu === 'identitas' ? (
              <IdentitasSekolah data={dataTentang?.identitas} />
            ) : menu === 'tujuan' || menu === 'hasil' || menu === 'sasaran' ? (
              <ProfilSekolah data={dataTentang?.profil} jenis={menu} />
            ) : (
              <ComingSoonPage />
            )}
          </div>
